                                                            ...
                                                            <td>
                                                                <c:choose>
                                                                    ...
                                                                    <c:when test="${request_course.getStatus() == '2'}">
                                                                        <p class="text-primary">Accept</p>
                                                                    </c:when>
                                                                    <c:when test="${request_course.getStatus() == '3'}">
                                                                        <p class="text-primary">Reject</p>
                                                                    </c:when>
...
<!--                                                                <button type="button" class="btn btn-outline-secondary" onclick="window.location.href = 'update_request_mentor?request_id=${request_course.getRepc_id()}'">Details</button>-->
                                                                <button type="button" class="btn btn-outline-success" onclick="promptReason('accept', ${request_course.getRepc_id()})">Accept</button>
                                                                <button type="button" class="btn btn-outline-danger" onclick="promptReason('reject', ${request_course.getRepc_id()})">Reject</button>
                                                            </td>
                                                        </tr>
                                                    </c:forEach>
                                                </tbody>
        ...
        <script>
                                                                    function promptReason(action, requestId) {
                                                                        var reason = prompt("Please enter the reason for " + action + ":");
                                                                        if (reason !== null && reason.trim() !== "") {
                                                                            // Gửi dữ liệu đến server
                                                                            sendRequest(action, requestId, reason);
                                                                        } else {
